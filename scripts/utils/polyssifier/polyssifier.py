...
def poly(data, label, groups=None, n_folds=10, scale=True, exclude=[],
    ...
    groups      = numpy vector that specifies the group label for each sample (for group k-fold)
    ...
    if concurrency == 1:
        ...
        logger.info('Starting pool.starmap()')
...
def fit_clf(args, clf_name, val, n_fold, project_name, save, scoring):
    ...
    '''
    logger.info(f'Starting fold {n_fold} for classifier {clf_name}')
