...
for SNR in SNRs:
    ...
    for graph_ix in range(num_graphs):
        ...
        #Generate a number of adjacency matrices
        for adj_ix in range(num_adjacency):
            A = graph2adj(g)
            u_rate = 1


            #Using the adjacency matrices and graphs, generate the noise matrix for all subjects until converged
            ...
            report1 = poly(data_scaled1, np.array(labels_sr1), n_folds=5, scale=True, random_state=random_state,
                                ...
                                scoring='auc', project_name=f'SR1_noise_{scalar}', concurrency=4, verbose=True)
            

            report2 = poly(data_scaled2, np.array(labels_sr2), n_folds=5, scale=True, random_state=random_state,
                                exclude=['Decision Tree', 'Random Forest', 'Voting', 'Nearest Neighbors', 'Linear SVM'], 
                                scoring='auc', project_name=f'SR2_noise_{scalar}', concurrency=4, verbose=True)
            
            report3 = poly(data_scaled3, np.array(labels_concat), n_folds=5, scale=True, random_state=random_state,
                                exclude=['Decision Tree', 'Random Forest', 'Voting', 'Nearest Neighbors', 'Linear SVM'],
                                scoring='auc', project_name=f'CONCAT_noise_{scalar}', concurrency=4, verbose=True)
