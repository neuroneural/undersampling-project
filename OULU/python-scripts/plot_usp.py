   ...
   for name, group1 in grouped1:
      ...
      for idx, (label, data1) in enumerate(group1.groupby('nstd')):
         if lower_limit is None:
            ...
            plt.plot(data_filtered['snr'], data_filtered['auc'], marker='o', linestyle='-', color='green', label='TR=100ms')
      ...
      for idx, (label, data2) in enumerate(grouped2.get_group(name).groupby('nstd')):
         if lower_limit is None:
            ...
            plt.plot(data_filtered['snr'], data_filtered['auc'], marker='s', linestyle='--', color='red', label='TR=2150ms')
...
def count_winners(dataframes, lower_limit=0):
    ...
    sampling_rates = ['TR=100ms', 'TR=2150ms', 'Concat', 'Add']
...
#plot_winners(win_df, lower_limit=-0.5, filename='winners_lim.png')
