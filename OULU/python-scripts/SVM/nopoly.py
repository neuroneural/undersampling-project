...
for SNR in SNRs:
    ...
    for j in range(n_sections):
                ...
                sr1_start_ix += sr1_start_ix
                sr1_end_ix = sr1_end_ix + sr1_start_ix
    ...
    for fold_number, (train_index, test_index) in enumerate(sgkf.split(X_sr1, y_sr1, group_sr1), start=1):
        ...
        svm_sr1.fit(sr1_train.reshape(len(sr1_train), -1), y_train_sr1)
        svm_sr2.fit(sr2_train.reshape(len(sr2_train), -1), y_train_sr2)
        ...
        fold_roc_sr1 = roc_auc_score(y_test_sr1, svm_sr1.predict_proba(sr1_test.reshape(len(sr1_test), -1))[:, 1])
        fold_roc_sr2 = roc_auc_score(y_test_sr2, svm_sr2.predict_proba(sr2_test.reshape(len(sr2_test), -1))[:, 1])
    ...
    df1.to_pickle(f'/data/users2/jwardell1/undersampling-project/HCP/pkl-files/sr1_{SNR}_nopoly.pkl')
    df2 = pd.DataFrame(res2)
    df2.to_pickle(f'/data/users2/jwardell1/undersampling-project/HCP/pkl-files/sr2_{SNR}_nopoly.pkl')
    df3 = pd.DataFrame(res3)
    df3.to_pickle(f'/data/users2/jwardell1/undersampling-project/HCP/pkl-files/concat_{SNR}_nopoly.pkl')
    df4 = pd.DataFrame(res4)
    df4.to_pickle(f'/data/users2/jwardell1/undersampling-project/HCP/pkl-files/add_{SNR}_nopoly.pkl')