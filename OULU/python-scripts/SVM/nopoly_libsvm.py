...
for SNR in SNRs:
    ...
    for noise_ix in range(num_noise):
        ...
        for fold_number, (train_index, test_index) in enumerate(sgkf.split(X_tr100, y_tr100, group_tr100), start=0):
            ...
            fold_roc_sr1 = roc_auc_score(y_test_sr1, np.array(p_vals_sr1)[:, 1])
            fold_roc_sr2 = roc_auc_score(y_test_sr2, np.array(p_vals_sr2)[:, 1])
            fold_roc_add = roc_auc_score(y_test_add, np.array(p_vals_add)[:, 1])
            fold_roc_concat = roc_auc_score(y_test_concat, np.array(p_vals_concat)[:, 1])
