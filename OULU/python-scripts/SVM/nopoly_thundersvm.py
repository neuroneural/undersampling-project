...
for SNR in SNRs:
    for noise_ix in range(num_noise):
        ...
        for fold_number, (train_index, test_index) in enumerate(sgkf.split(X_tr100, y_tr100, group_tr100), start=0):
            ...
            y_train_sr1, y_test_sr1 = np.array(y_tr100)[train_index], np.array(y_tr100)[test_index]
            y_train_sr2, y_test_sr2 = np.array(y_tr2150)[train_index], np.array(y_tr2150)[test_index]
            ...
            svm_sr1 = SVC(kernel='linear', C=1.0)
            svm_sr1.fit(sr1_train, y_train_sr1)
            y_pred_sr1 = svm_sr1.predict(sr1_test)
            fold_roc_sr1 = roc_auc_score(y_test_sr1, np.array(y_pred_sr1)[:, 1])

            svm_sr2 = SVC(kernel='linear', C=1.0)
            svm_sr2.fit(sr2_train, y_train_sr2)
            y_pred_sr2 = svm_sr2.predict(sr2_test)
            fold_roc_sr2 = roc_auc_score(y_test_sr2, np.array(y_pred_sr2)[:, 1])

            svm_add = SVC(kernel='linear', C=1.0)
            svm_add.fit(add_train, y_train_add)
            y_pred_add = svm_add.predict(add_test)
            fold_roc_add = roc_auc_score(y_test_add, np.array(y_pred_add)[:, 1])


            svm_concat = SVC(kernel='linear', C=1.0)
            svm_concat.fit(concat_train, y_train_concat)
            y_pred_concat = svm_concat.predict(concat_test)
            fold_roc_concat = roc_auc_score(y_test_concat, np.array(y_pred_concat)[:, 1])
        ...
        df1.to_pickle(f'/data/users2/jwardell1/undersampling-project/OULU/pkl-files/sr1_{SNR}_libsvm.pkl')
        df2 = pd.DataFrame(res2)
        df2.to_pickle(f'/data/users2/jwardell1/undersampling-project/OULU/pkl-files/sr2_{SNR}_libsvm.pkl')
        df3 = pd.DataFrame(res3)
        df3.to_pickle(f'/data/users2/jwardell1/undersampling-project/OULU/pkl-files/concat_{SNR}_libsvm.pkl')
        df4 = pd.DataFrame(res4)
        df4.to_pickle(f'/data/users2/jwardell1/undersampling-project/OULU/pkl-files/add_{SNR}_libsvm.pkl')
